...
def _html_begin_table_without_header(colwidths_ignore, colaligns_ignore):
	...
	return "\n".join(["<table class=tabulate>", "<tbody>"])


def escape(textcontent):
...
def _html_row_with_attrs(celltag, cell_values, colwidths, colaligns):
	...
	values_with_attrs = ["<{0}{1}>{2}</{0}>".format(celltag, alignment.get(a, ''), escape(c.strip()) )  for c, a in zip(cell_values, colaligns)]
	rowhtml = "<tr>" + "".join(values_with_attrs).rstrip() + "</tr>"
	if celltag == "th":  # it's a header row, create a new table header
		rowhtml = "\n".join(["<table class=tabulate>",
							 "<thead>",
							 rowhtml,
							 "</thead>",
							 "<tbody>"])
...
def tabulate(tabular_data, headers=(), tablefmt="simple",
	...
	if override_cols_fmt:
		cols = [[_format(v, ct, floatfmt if not f else f, missingval, has_invisible) for v in c]
				 for c,ct,f in zip(cols, coltypes, override_cols_fmt)]
	else:
		cols = [[_format(v, ct, floatfmt, missingval, has_invisible) for v in c]
				 for c,ct in zip(cols, coltypes)]

	# align columns
	if override_cols_align:
		aligns = override_cols_align
	else:
